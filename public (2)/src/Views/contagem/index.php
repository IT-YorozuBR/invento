...
<div class="form-container">
    ...
    <form id="formContagem" method="POST" action="?pagina=contagem">
        ...
        <button type="submit" class="btn btn-primary" style="margin-top:8px;" id="btnRegistrar"
            data-loading-text="Registrando..." data-success-text="Registrado!"
            data-error-text="Erro!" data-btn-anim="spinner">
            <span class="btn-text">
        ...
        <form method="GET" action="?pagina=contagem"
            ...
            <div style="display:flex;gap:8px;align-items:flex-end;">
                <button type="submit" class="btn btn-primary btn-sm"
                    data-loading-text="Filtrando..." data-btn-anim="dots">
                    <span class="btn-text"><i class="fas fa-filter"></i> Filtrar</span>
                </button>
            ...
            <table>
                <!-- <div style="display:flex;justify-content:flex-end;margin-bottom:10px;">
                    <button class="btn btn-secondary btn-sm"
                        ...
                        id="btnRecarregarLista"
                        data-loading-text="Atualizando..."
                        data-success-text="Atualizado!"
                        data-error-text="Erro!"
                        data-btn-anim="spinner">
                        <span class="btn-text">
                            <i class="fas fa-rotate"></i> Atualizar lista
                        </span>
                    </button>
                </div> -->
            ...
            <?php else: ?>
                ...
                <?php foreach ($pagination['items'] as $c): ?>
                    ...
                    if ($finalizado) {
                        ...
                        // Forçar status visual baseado no número de contagens
                        if ($status === 'concluida') {
                            $badge = '<span class="status-badge status-concluida"><i class="fas fa-check-circle"></i> Concluída</span>';
                        } elseif ($status === 'divergente') {
                            $badge = '<span class="status-badge status-divergente"><i class="fas fa-exclamation-triangle"></i> Divergente</span>';
                        } elseif ($status === 'terceira') {
                            $badge = '<span class="status-badge status-secundaria"><i class="fas fa-layer-group"></i> 3ª Contagem</span>';
                        } elseif ($status === 'secundaria') {
                            $badge = '<span class="status-badge status-secundaria"><i class="fas fa-layer-group"></i> 2ª Contagem</span>';
                        } else {
                            $badge = '<span class="status-badge status-primaria"><i class="fas fa-clock"></i> 1ª Contagem</span>';
                        }
                    }
    ...
    <div id="modalContent">
        ...
        <div class="modal-actions">
            <button id="btnNovaContagem" class="btn btn-primary"
                data-loading-text="Liberando..." data-success-text="Liberada!"
                data-error-text="Erro!" data-btn-anim="progress">
                <span class="btn-text"><i class="fas fa-plus"></i> Liberar Próxima Contagem</span>
            </button>
            <button id="btnFinalizar" class="btn btn-danger"
                data-loading-text="Encerrando..." data-success-text="Encerrada!"
                data-error-text="Erro!" data-btn-anim="pulse">
                <span class="btn-text"><i class="fas fa-lock"></i> Encerrar Contagem</span>
            </button>
            <button onclick="fecharAcaoModal()" class="btn btn-ghost" data-no-state>
...
<script>
    ...
    function executarLiberar(contagemId, fase) {
        ...
        const btn = document.getElementById('btnNovaContagem');
        ...
        showConfirm(
         ...
         <small style="color:var(--gray)">O operador poderá registrar a próxima contagem.</small>`,
            async () => {
                ...
                const data = await fetchWithState(btn, `?pagina=ajax&acao=${acao}`, {
                    method: 'POST',
                    body: fd
                });
    ...
    function executarEncerrar(contagemId, partnumber) {
        ...
        showConfirm(
         ...
         <small style="color:var(--gray)">Esta ação não pode ser desfeita.</small>`,
            async () => {
                const fd = new FormData();
                fd.append('csrf_token', csrfToken);
                fd.append('contagem_id', contagemId);

                const data = await fetchWithState(btn, '?pagina=ajax&acao=finalizar_contagem', {
                    method: 'POST',
                    body: fd
                });
                if (data?.success) setTimeout(() => location.reload(), 1600);
    ...
    async function recarregarLista() {
        const btn = document.getElementById('btnRecarregarLista');
        ...
        try {
            ...
            const novaLista = temp.querySelector('#listaContagens');

            if (novaLista) {
                container.innerHTML = novaLista.innerHTML;
            }

            btnLoading(btn, false, true);
        } catch (e) {
            btnLoading(btn, false, false);
            showToast('Erro ao atualizar a lista', 'erro');
        }
    }
</script>
